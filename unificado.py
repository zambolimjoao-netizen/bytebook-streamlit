...
def validar_json_vs_df(json_data, df):
    ...
    for index, json_item in enumerate(json_data):
        ...
        part_number_json = json_item['codigosInterno'][0] if json_item['codigosInterno'] else ''
        # Encontrar a coluna PART_NUMBER dinamicamente no DataFrame
        col_part_number_df = encontrar_coluna(df, "PART_NUMBER")
        part_number_df = df_row.get(col_part_number_df, "")
        ...
        # Contar atributos no DataFrame para a linha atual
        colunas_atributos_df = [col for col in df.columns if col.upper().startswith('ATT_')]
        atributos_no_df = 0
        for col in colunas_atributos_df:
            if pd.notna(df_row.get(col)):
                atributos_no_df += 1
        
        # Contar atributos no JSON
        atributos_no_json = len(json_item.get('atributos', []))

        if atributos_no_df != atributos_no_json:
            return False, f"Erro na linha {index+2} (Part Number: {part_number_df}): A contagem de atributos não corresponde (Planilha: {atributos_no_df}, JSON: {atributos_no_json})."
    ...
    if uploaded_files:
        ...
        with col1:
            if st.button("Limpar Lista de Arquivos"):
                ...
                st.rerun()
        with col2:
            if st.button("Expandir Todos"):
                st.session_state.expand_all = True
        with col3:
            if st.button("Recolher Todos"):
                st.session_state.expand_all = False
        ...
        if st.session_state.generated_jsons:
            ...
            if len(st.session_state.generated_jsons) > 1:
                ...
                st.download_button(
                    label="Baixar Todos os JSONs (.zip)",
